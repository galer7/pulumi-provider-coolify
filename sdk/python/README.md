...
This boilerplate creates a working Pulumi-owned provider named `coolify`.
...
1. Set the following options:
   * Owner: pulumi 
   * Repository name: pulumi-coolify-native (replace "coolify" with the name of your provider)
   * Description: Pulumi provider for coolify
...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `coolify` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `galer7` with the `ORG` of your provider.
   - replace all instances of the `github.com/galer7/pulumi-provider-coolify` repository with the `REPOSITORY` location
...
1. A `provider/` folder containing the building and implementation logic
    1. `cmd/pulumi-resource-coolify/main.go` - holds the provider's sample implementation logic.
2. `deployment-templates` - a set of files to help you around deployment and publication
3. `sdk` - holds the generated code libraries created by `pulumi-gen-coolify/main.go`
